...
/**
 * Implementation of the node using the log-likelihood (LL) score to compute the value of the Alphas.
 *
 */
public class NodeLL extends NodeAbstract{
	...
	/**
	 * Constructor that adds the parent identification to the previous one.
	 * Delegates to the constructor of the super class.
	 * @param training Double array containing the training dataset.
	 * @param max_values Array that contains the maximum values for each attribute and class.
	 * @param node_id Node identification.
	 * @param parent Parent identification.
	 */
	...
	/**
	 * Computes the value of the Alpha parameters accordingly to the LL score.
	 */
	@Override
	public void computeAlpha() {
		...
		for(int i = 0; i < training[0].length-1; i++) {
			...
			//Computes the values of the alpha for a given node(self)->node(target) using the LL score
			double total = 0;
